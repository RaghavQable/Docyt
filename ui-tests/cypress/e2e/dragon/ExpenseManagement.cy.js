...
const table=require("../../lib/common/webtables/tables");
...
describe("Expense Management", () => {
    ...
    it("C9171: Submit Expense",{tags: '@smoke'},()=>
    {
        const current_date = time_helper.get_current_date_of_month();
        const amount = number_helper.get_random_number(100000);
        const formatted_amount = number_helper.get_formatted_amount(amount);
    ...
    it("C336435: Verify that the original expense receipt is visible in the 'Related Document' Section of corresponding expense report",{ tags: '@smoke'},()=>
        {
            const current_date = time_helper.get_current_date_of_month();
            const amount = number_helper.get_random_number(100000);
            const formatted_amount = number_helper.get_formatted_amount(amount);
            ...
            expense_page.filter_by_amount(amount.toString(), amount.toString());
            expense_page.verify_expense_merchant(formatted_amount, expense["merchant"]);
            invoice_queue_page.createTableAlias2();
            table.first_row_table_td_text_present('@my_expenses_table','Total Energy Management Inc.')
        })
